...
const SignUp = () => {
	...
	const handleSignUp = () => {
		if ( first_name === '' || last_name === '' || email === '' || phone_number === '' || password === '' || cpassword === ''){
			alert("You have a missing field")
		}else{
			if(password !== cpassword){
				alert("Passwords do not match")
			}else{
				dispatch(signup({first_name:first_name,last_name:last_name,email:email,phone_number:phone_number,password:password}))
				setFirstName('')
				setLastName('')
				setEmail('')
				setPhoneNumber('')
			...
			<div className="bg-white/80 shadow-md backdrop-blur-md rounded-md md:w-[30%] p-4">
				...
				<div className="flex flex-col justify-center py-2 place-items-center">
					<div className="w-full flex py-2 gap-5 flex-col items-center">
						...
						<button onClick={handleSignUp} className="bg-black text-white md:max-w-[450px] md:w-[400px] py-3 rounded-md hover:bg-green-600 duration-500">
